...
function te({ groupSize: e, panelElement: t, styleProp: n }) {
	let o;
	const [i, s] = zt(n);
	switch (s) {
		case "%":
...
function ye(e) {
	...
	return n === 0 ? t.map((o) => ({
		...
		let l = 0;
		if (s.collapsedSize !== void 0) l = D(te({
			groupSize: n,
			panelElement: i,
			styleProp: s.collapsedSize
		}) / n * 100);
		let r;
		if (s.defaultSize !== void 0) r = D(te({
			groupSize: n,
			panelElement: i,
			styleProp: s.defaultSize
		}) / n * 100);
		let a = 0;
		if (s.minSize !== void 0) a = D(te({
			groupSize: n,
			panelElement: i,
			styleProp: s.minSize
		}) / n * 100);
		let u = 100;
		if (s.maxSize !== void 0) u = D(te({
			groupSize: n,
			panelElement: i,
			styleProp: s.maxSize
		}) / n * 100);
...
function wt({ orientation: e, rects: t, targetRect: n }) {
	const o = {
		x: n.x + n.width / 2,
		y: n.y + n.height / 2
	};
...
function Ye(e) {
	...
	{
		let x = -1;
		for (const c of s) c.hasAttribute("data-panel") && (x++, c.hasAttribute("data-disabled") || (g++, u === -1 && (u = x), f = x));
	}
	if (g > 1) {
		let x = -1;
		for (const c of s) if (c.hasAttribute("data-panel")) {
			x++;
			...
			if (p) {
				if (d) {
					...
					} else z.length ? v = z : v = [n === "horizontal" ? new DOMRect(m.right, S.top, S.left - m.right, S.height) : new DOMRect(S.left, m.bottom, S.width, S.top - m.bottom)];
					for (const y of v) {
...
function Y({ overrideDisabledPanels: e, panelConstraints: t, prevSize: n, size: o }) {
	const { collapsedSize: i = 0, collapsible: s, disabled: l, maxSize: r = 100, minSize: a = 0 } = t;
	if (l && !e) return n;
	if (G(o, a) < 0) if (s) {
		const u = (i + a) / 2;
		G(o, u) < 0 ? o = i : o = a;
...
function se({ delta: e, initialLayout: t, panelConstraints: n, pivotIndices: o, prevLayout: i, trigger: s }) {
	if (I(e, 0)) return t;
	...
	switch (s) {
		case "keyboard":
			{
				...
				const { collapsedSize: m = 0, collapsible: S, minSize: v = 0 } = p;
				if (S) {
					const y = r[c];
		...
		default: {
			...
			if (S && G(m, y) < 0) if (e > 0) {
				const h = y - v, P = h / 2;
				G(m + e, y) < 0 && (e = G(e, P) <= 0 ? 0 : h);
			} else {
				const h = y - v, P = 100 - h / 2;
				G(m - e, y) < 0 && (e = G(100 + e, P) > 0 ? 0 : -h);
			}
			break;
		}
	}
	{
		...
		for (;;) {
			...
			if (m += h, p += c, p < 0 || p >= n.length) break;
		}
	...
	}
	{
		...
		for (; p >= 0 && p < n.length;) {
			...
			if (!I(S, y) && (d += S - y, u[p] = y, d.toFixed(3).localeCompare(Math.abs(e).toFixed(3), void 0, { numeric: !0 }) >= 0)) break;
			e < 0 ? p-- : p++;
		}
	}
	if (Ot(a, u)) return i;
	{
		...
		if (u[c] = S, !I(S, m)) {
			...
			for (; h >= 0 && h < n.length;) {
				...
				if (I(P, C) || (v -= C - P, u[h] = C), I(v, 0)) break;
				e > 0 ? h-- : h++;
			}
		}
	}
	if (!I(Object.values(u).reduce((c, p) => p + c, 0), 100, .1)) return i;
	const x = Object.keys(i);
	return u.reduce((c, p, m) => (c[x[m]] = p, c), {});
}
function V(e, t) {
	if (Object.keys(e).length !== Object.keys(t).length) return !1;
	for (const n in e) if (t[n] === void 0 || G(e[n], t[n]) !== 0) return !1;
	return !0;
}
function B({ layout: e, panelConstraints: t }) {
	const n = Object.values(e), o = [...n], i = o.reduce((r, a) => r + a, 0);
	if (o.length !== t.length) throw Error(`Invalid ${t.length} panel layout: ${o.map((r) => `${r}%`).join(", ")}`);
	if (!I(i, 100) && o.length > 0) for (let r = 0; r < t.length; r++) {
		const a = o[r];
	...
	if (!I(s, 0)) for (let r = 0; r < t.length; r++) {
		const a = o[r];
		L(a != null, `No layout data found for index ${r}`);
		const u = a + s, f = Y({
			overrideDisabledPanels: !0,
		...
		if (a !== f && (s -= f - a, o[r] = f, I(s, 0))) break;
	}
	const l = Object.keys(e);
	return o.reduce((r, a, u) => (r[l[u]] = a, r), {});
}
function et({ groupId: e, panelId: t }) {
	const n = () => {
		const r = U();
		for (const [a, { defaultLayoutDeferred: u, derivedPanelConstraints: f, layout: g, groupSize: d, separatorToPanels: z }] of r) if (a.id === e) return {
		...
		const a = s();
		if (r === a) return;
		const { defaultLayoutDeferred: u, derivedPanelConstraints: f, group: g, groupSize: d, layout: z, separatorToPanels: x } = n(), c = g.panels.findIndex((v) => v.id === t), p = c === g.panels.length - 1, S = B({
			layout: se({
				delta: p ? a - r : r - a,
				initialLayout: z,
				panelConstraints: f,
	...
	return {
		...
		getSize: () => {
			...
			return {
				asPercentage: a,
				inPixels: r.orientation === "horizontal" ? u.offsetWidth : u.offsetHeight
			};
		},
		isCollapsed: () => {
		...
		resize: (r) => {
			const { group: a } = n(), { element: u } = i(), f = Q({ group: a });
			l(D(te({
				groupSize: f,
				panelElement: u,
				styleProp: r
...
function tt({ groupId: e }) {
	...
	return {
		...
		setLayout(n) {
			const { defaultLayoutDeferred: o, derivedPanelConstraints: i, group: s, groupSize: l, layout: r, separatorToPanels: a } = t(), u = B({
				layout: n,
				panelConstraints: i
			});
...
function H(e, t) {
	const n = fe(e), o = $(n.id, !0), i = n.separators.find((g) => g.element === e);
	L(i, "Matching separator not found");
	const s = o.separatorToPanels.get(i);
	L(s, "Matching panels not found");
	const l = s.map((g) => n.panels.indexOf(g)), a = tt({ groupId: n.id }).getLayout(), f = B({
		layout: se({
			delta: t,
			initialLayout: a,
...
function Oe(e) {
	...
	if (!n.disabled) switch (e.key) {
		...
		case "Enter": {
			e.preventDefault();
			const o = fe(t), { derivedPanelConstraints: s, layout: l, separatorToPanels: r } = $(o.id, !0), a = o.separators.find((d) => d.element === t);
			...
			if (L(g, "Panel metadata not found"), g.collapsible) {
				const d = l[f.id];
				H(t, (g.collapsedSize === d ? o.mutableState.expandedPanelSizes[f.id] ?? g.minSize : g.collapsedSize) - d);
			}
			break;
		}
		case "F6": {
...
function Ge(e) {
	...
	let i = !1;
	n.forEach((s) => {
		s.separator && (i || (i = !0, s.separator.element.focus({ preventScroll: !0 })));
...
var Nt = (e) => e, me = () => {}, nt = 1, ot = 2, it = 4, rt = 8, Ae = 3, Ne = 12;
var ce;
function _e() {
...
function _t({ cursorFlags: e, groups: t, state: n }) {
	let o = 0, i = 0;
	switch (n) {
		case "active":
		...
		switch (n) {
			case "active":
				if (e && _e()) {
					...
					if (s) return r ? "se-resize" : a ? "ne-resize" : "e-resize";
					if (l) return r ? "sw-resize" : a ? "nw-resize" : "w-resize";
...
function st({ document: e, event: t, hitRegions: n, initialLayoutMap: o, mountedGroups: i, pointerDownAtPoint: s, prevCursorFlags: l }) {
	let r = 0;
	n.forEach((u) => {
		...
		if (!p || !m) return;
		const { defaultLayoutDeferred: S, derivedPanelConstraints: v, groupSize: y, layout: h, separatorToPanels: P } = m;
		if (v && h && P) {
			const M = se({
				delta: c,
				initialLayout: p,
...
function Be(e) {
	...
	if (i !== 0) {
		const s = D((100 - n) / i);
		for (const l of e) l.defaultSize === void 0 && (o[l.panelId] = s);
	}
	return o;
}
function Ft(e, t, n) {
...
function jt({ group: e, nextGroupSize: t, prevGroupSize: n, prevLayout: o }) {
	...
	if (!l || a.length === 0) return o;
	const u = 100 - i, f = { ...o };
	if (r.forEach((g, d) => {
		f[d] = g;
	}), s > 0) for (const g of a) f[g] = D((o[g] ?? 0) / s * u);
	else {
		const g = D(u / a.length);
		for (const d of a) f[d] = g;
	}
	return f;
}
function Ht(e, t) {
...
function Vt(e) {
		...
		for (const p of c) {
			...
			if (S === e.element) {
				if (t) {
					...
					const y = $(e.id);
					if (!y) return;
					const h = ye(e), P = y.defaultLayoutDeferred ? Be(h) : y.layout, C = B({
						layout: jt({
							group: e,
							nextGroupSize: v,
					...
					if (!y.defaultLayoutDeferred && V(y.layout, C) && $t(y.derivedPanelConstraints, h) && y.groupSize === v) return;
					_(e, {
	...
	const g = B({
		layout: e.mutableState.layouts[a] ?? u ?? Be(r),
		panelConstraints: r
	...
	return Ye(e).forEach((c) => {
		...
		t = !1, q.set(d, Math.max(0, (q.get(d) ?? 0) - 1)), Lt(e), e.separators.forEach((p) => {
			p.element.removeEventListener("keydown", Oe);
		}), q.get(d) || (d.removeEventListener("dblclick", Te, !0), d.removeEventListener("pointerdown", Ge, !0), d.removeEventListener("pointerleave", $e), d.removeEventListener("pointermove", je), d.removeEventListener("pointerout", He), d.removeEventListener("pointerup", Ve, !0)), s.disconnect();
	};
}
function Bt() {
...
function Ut({ children: e, className: t, defaultLayout: n, disableCursor: o, disabled: i, elementRef: s, groupRef: l, id: r, onLayoutChange: a, onLayoutChanged: u, orientation: f = "horizontal", resizeTargetMinimumSize: g = {
	coarse: 20,
	fine: 10
}, style: d, ...z }) {
	...
	const M = oe((b, w) => {
		...
		registerPanel: (b) => {
			const w = h.current;
			return w.panels = Se(f, [...w.panels, b]), y(), () => {
				w.panels = w.panels.filter((E) => E !== b), y();
			};
		},
		registerSeparator: (b) => {
			const w = h.current;
			return w.separators = Se(f, [...w.separators, b]), y(), () => {
	...
	}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(at.Provider, {
		value: X,
	...
	if (i) try {
		...
		if (t) {
			...
			if (r && Array.isArray(r.layout) && t.length === r.layout.length) {
				const a = {};
				for (let u = 0; u < t.length; u++) a[t[u]] = r.layout[u];
				return a;
			}
		} else {
			...
			if (l.length === 1) {
				...
				if (r && Array.isArray(r.layout)) {
					...
					if (a.length === r.layout.length) {
						const u = {};
						for (let f = 0; f < a.length; f++) u[a[f]] = r.layout[f];
						return u;
					}
				}
			}
...
function on({ debounceSaveMs: e = 100, panelIds: t, storage: n = localStorage, ...o }) {
	const i = t !== void 0, s = "id" in o ? o.id : o.groupId, l = de(s, t ?? []), r = (0, import_react.useSyncExternalStore)(Xt, () => n.getItem(l), () => n.getItem(l)), a = (0, import_react.useMemo)(() => {
		if (r) {
		...
		t,
		n
	]), f = a ?? u, g = (0, import_react.useRef)(null), d = (0, import_react.useCallback)(() => {
		const c = g.current;
	...
	const z = (0, import_react.useCallback)((c) => {
		d();
		let p;
...
function Yt({ children: e, className: t, collapsedSize: n = "0%", collapsible: o = !1, defaultSize: i, disabled: s, elementRef: l, groupResizeBehavior: r = "preserve-relative-size", id: a, maxSize: u = "100%", minSize: f = "0%", onResize: g, panelRef: d, style: z, ...x }) {
	const c = !!a, p = we(a), m = Le({ disabled: s }), S = (0, import_react.useRef)(null), v = Pe(S, l), { getPanelStyles: y, id: h, orientation: P, registerPanel: M, togglePanelDisabled: C } = Ce(), X = g !== null, F = oe((R, k, O) => {
		g?.(R, a, O);
	});
	K(() => {
		const R = S.current;
	...
	}, w = (0, import_react.useSyncExternalStore)((R) => ve(h, R), b, b);
	let E;
	return w ? E = JSON.parse(w) : i ? E = {
		flexGrow: void 0,
		flexShrink: void 0,
		flexBasis: i
	} : E = { flexGrow: 1 }, /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		...x,
...
function Qt({ children: e, className: t, disabled: n, elementRef: o, id: i, style: s, ...l }) {
	const r = we(i), a = Le({ disabled: n }), [u, f] = (0, import_react.useState)({}), [g, d] = (0, import_react.useState)("inactive"), z = (0, import_react.useRef)(null), x = Pe(z, o), { disableCursor: c, id: p, orientation: m, registerSeparator: S, toggleSeparatorDisabled: v } = Ce(), y = m === "horizontal" ? "vertical" : "horizontal";
	K(() => {
		const P = z.current;
		if (P !== null) {
			const M = {
	...
	let h;
	return n && !c && (h = "not-allowed"), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		...l,
...
//#endregion
export { Ut as Group, Yt as Panel, Qt as Separator, Pt as isCoarsePointer, on as useDefaultLayout, rn as useGroupCallbackRef, sn as useGroupRef, an as usePanelCallbackRef, ln as usePanelRef };
