...
One role per colour, 16-color ANSI only. Palette table (plan 043 post-audit):
    ...
    Role                       Colour          Weight
    COMMIT_ID + c: glyph       cyan            bold
    TRACE_ID + t: glyph        magenta         bold
    COMMIT_SUBJECT             default         bold
    TRACE_NAME                 default         bold
    ADDED (+)                  green
    MODIFIED (~)               yellow
    DELETED (-)                red
    RENAMED (rotate-glyph)     blue
    COVERAGE_GOOD (>=75%)      green
    COVERAGE_PARTIAL (>=50%)   yellow
    COVERAGE_POOR (<50%)       red
    DIM (paths/timestamps)     bright-black
    PRE_AUDIT                  bright-black (italic)
...
# role -> ANSI code sequence (SGR). 16-color only.
_CODES: dict[Role, str] = {
    Role.COMMIT_ID:        "\x1b[1;36m",   # bold cyan
    Role.TRACE_ID:         "\x1b[1;35m",   # bold magenta
    Role.COMMIT_SUBJECT:   "\x1b[1m",      # bold default
    Role.TRACE_NAME:       "\x1b[1m",      # bold default
