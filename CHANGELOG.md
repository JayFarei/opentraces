...
### Seal family — workflow, dataset, capsule on one projection contract (#178)

The seal-family train lands the contract that everything built on captured evidence is a pure function of content-addressed inputs — `projection = f(scope_ref, transform@digest, bucket_state@digest, answers)` — with exactly two seals (dataset, capsule), one clearance predicate on every egress door, and honesty labels that may never over-claim. No `bucket_digest` change; one additive schema `MINOR` (0.7.0 → 0.8.0).

- **One trustworthy engine (#179).** ADR-0008 (the contract + trust lattice); a single egress-clearance predicate (`core/egress_clearance.py`) shared by every door; an isolated-subprocess primitive (`core/isolation.py`) with an honest `sandbox_tier`; the workflow engine collapsed to one runner (`execute_workflow`, one versioned `opentraces.workflow.run_packet.v1`, the dead headless executor deleted); the rc=10 judgment handshake with recorded answers; workflow integrity (install rejects non-allowlisted dotfiles, run-time digest re-verify + `--strict`, trust-by-provenance for bucket access); ctx/trail-aware sanitization (`sanitize_companion_dict`, `SECURITY_VERSION` → 0.8.0, tail-probe leak 5/8 → 1/8). (#143 #166 #182–#189)
- **Explainable, shareable projections (#180).** Dataset: bind-time validation (no `sha256:unconfigured`), first-class `trace:A-B` row lineage + contract-triple provenance v2, watermark cursor + `dataset run --sync`, `dataset verify` (reproduces / bucket-advanced / integrity-failure), publish through the shared clearance predicate. Capsule: `capsule create/get/import` on the v7 ref grammar, honesty front-matter + `pushed` stamp, a redacted mini-bucket wiring the sanitization capability, seal-time preview/approve, clearance-gated publish, and a stateless render worker (deploy excluded). (#144 #167 #190–#199)
- **Replay honesty (#181).** A capsule's replay reports four honest properties — `reproducible` / `gradable` / `scoped` / `sandboxed` — each derived from locally-verifiable state (never a producer's self-label), with `verdict_trust` (the weakest-link min over the ratified lattice) as a derived summary; on today's corpus every capsule honestly floors and refuses "reproducible". Slice-scoped diff, oracle widening (captured_pass + declared-test UX), bundle secret-scan publish gate + sandbox v1, and additive `Environment` dependency-pin / interpreter / arch schema fields (the one `MINOR`, 0.7.0 → 0.8.0). (#146 #154–#157 #200 #201)
...
- **Zero-touch OTel context capture (#158).** The watcher tick now auto-flushes a project's active OTel-captured Context Tree sessions into the bucket once each session goes idle (watermark-gated, at most once per session, reads only the cheap per-session snapshot), so live captures land on the trace with no manual `capture-otlp flush`. The OTLP receiver buffer captures per-llm-request body refs so the live daemon snapshot path produces one full ContextNode per llm-request (not a collapsed per-turn node), and `ctx show --full --remote` lazy-fetches missing message blobs from the remote backend (verifying the `content_hash` round-trip and warming the local cache).
