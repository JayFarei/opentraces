...
- Status counts (2026-06-10, post-Phase-B): 16 verified, 19 partial,
  14 open, 7 tracked, 0 waived — 56 rows.
...
| CAP-4 | Global tracking is opt-out; repos auto-enroll on first capture; `excluded` marker respected | A | open | — | — |
| CAP-5 | `init --import-existing` backfills historical Claude sessions | A | open | — | — |
| CAP-6 | Hook failures never block the agent session (always exit 0) | A | open | — | — |
...
| CAP-8 | Installers (`setup claude-code/codex-cli/pi/git`) are idempotent and preserve unrelated hooks | A | partial | pi-setup-dry-run, onboard-integrations, tests/otbox/test_idempotency_sweep.py | — |
...
| BKT-2 | `bucket replay --repo` reconstructs the canonical Git event ref byte-identically; the strong mirror-equality journey is quarantined | B | partial | bucket-replay | #25 |
| BKT-3 | Cross-machine byte-identity (gzip mtime=0 everywhere); the gold journey is quarantined, the two-fork determinism harness executes | B | partial | tests/otbox/test_determinism.py | #25 |
| BKT-4 | `bucket repair` is idempotent; `prune` never touches events or trace.json | B | open | — | #25 |
| BKT-5 | `bucket verify` detects corrupted blobs and dangling refs (needs fault-injected worlds) | B | open | — | #25 |
...
| ID | Claim | Axis | Status | Verifiers | Issue |
|---|---|---|---|---|---|
| DSC-1 | `trace query` returns bounded candidate packets from the BM25+semantic index without full transcript loads | C | verified | build-dataset-lineage-search, trace-map-and-slice | — |
| DSC-2 | `trace map --bursts` emits deterministic burst intent (trigger/spec/commit) | C | verified | trace-map-and-slice | — |
| DSC-3 | `trace slice` templates produce bounded packets | C | verified | trace-map-and-slice | — |
| DSC-4 | `--waste` / `--run-intel` / `trace compare` deterministic, derive-on-demand, byte-identical across map/get (compare is exit-0 smoke only; run-twice determinism never asserted) | C | partial | trace-compare-smoke | — |
| DSC-5 | Query latency acceptable on a mature repo (the Spotlight perf regression class); read-command budgets + search-eval harness execute, perf internals owned by the perf lanes | C | partial | tests/otbox/test_perf_budgets.py, tests/search_eval/test_search_eval.py | #40, #41 |
...
| TRL-4 | The 8 survival states are computed from real git history; a reverted edit shows `reverted` (probes kill the 0-results tautology; only `reverted` is exercised credibly) | D | partial | survival-walk-reverted, survival-walk | — |
...
| ID | Claim | Axis | Status | Verifiers | Issue |
|---|---|---|---|---|---|
| AGT-1 | Every public command supports `--json` with a structured envelope (sweep executes; 5 KNOWN_FINDINGS xfails outstanding) | H | partial | tests/cli/test_json_surface_sweep.py | #25 |
| AGT-2 | `--json` envelopes carry actionable `next_steps` forming a coherent graph to the documented goals | H | partial | tests/otbox/test_next_steps_walker.py | — |
| AGT-3 | Frozen envelope contracts (`opentraces.context_*.v1` etc.) never drift without a version bump | H | partial | tests/cli/test_json_surface_sweep.py::test_envelope_shapes_match_snapshot | — |
...
| AGT-5 | Documented exit codes (e.g. 6 unresolved ref, 3 schema-ahead, 2 contract) stable across releases | H | open | — | — |
...
| ID | Claim | Axis | Status | Verifiers | Issue |
|---|---|---|---|---|---|
| NF-1 | Determinism: same world, same command, byte-identical output (two-fork checkpoint harness; per-command run-twice not yet swept) | I | partial | tests/otbox/test_determinism.py | — |
