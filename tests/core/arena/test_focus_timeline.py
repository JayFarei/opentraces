...
def test_timeline_classifies_timezone_less_completion_as_corrupt(
    tmp_path: Path,
) -> None:
    bench = Bench(
        source=_scenario(tmp_path),
    ...
    with bench.run(app_state="install-only") as run:
        ...
        completion = next(row for row in rows if row["event"] == "action_completed")
        completion["recorded_at"] = "2099-01-01T00:00:00"
        timeline_path.write_text(
            "".join(json.dumps(row, sort_keys=True) + "\n" for row in rows),
            encoding="utf-8",
        )
    ...
    assert run.result["evidence"]["complete"] is True
    assert recordings["rewatchable"] is False
    assert recordings["timeline"] == {
        "complete": False,
        "path": "recordings/timeline.jsonl",
        "reason": (
            "execution timeline is corrupt: timeline recorded_at is not "
