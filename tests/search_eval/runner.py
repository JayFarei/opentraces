...
def _discovery_loop_smoke(plan: CorpusPlan, project_dir: Path, env: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        for verb, extra in (
            ...
            stage = "card" if extra == ["--card"] else verb
            if proc.returncode == 0:
                if extra == ["--card"]:
                    ...
                    if not card.get("headline") or not card.get("provenance_color"):
                        return {
                            ...
                            "stages": steps,
                            "failed_stage": "card",
                            "reason": "card missing headline/provenance_color",
                        }
                steps.append(stage)
                continue
            return {
                ...
                "stages": steps,
                "failed_stage": stage,
                "reason": (proc.stderr or proc.stdout or f"{stage} failed").strip()[:160],
            }
        discovery = _discover_smoke(plan, project_dir, env)
        if discovery.get("ok"):
            steps.append("discover")
        return {
            "ok": len(steps) >= 5 and discovery.get("ok", False),
            "trace_id": trace_id,
            "stages": steps,
            "discovery": discovery,
        }
