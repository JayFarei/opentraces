...
pytest.skip(
    "legacy TUI client perf lane is decommissioned until the next dataset "
...
@pytest.mark.perf
@pytest.mark.parametrize("scenario", load_scenarios("tui", lane="nightly"), ids=lambda s: s.name)
def test_tui_perf(
    scenario,
