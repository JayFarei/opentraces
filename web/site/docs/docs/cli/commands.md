...
- **Run compare** — `trace compare <a> <b>` emits `opentraces.trace_compare.v1`: per-side fidelity plus `{a, b, delta}` triples over Metrics, deterministic quality persona scores (skip with `--no-quality`), and burst/error/security signals, with both traces pinned to the same burst gap (`--burst-gap`, default 35).
