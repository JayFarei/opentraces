...
6. updates search projections used by `trace query` (including
   `trace query --skill <name>`), `trace map`, and `trace slice`.

The raw bucket is local-only unless you run `opentraces bucket connect` and sync
with `opentraces bucket sync push`.
