...
Parsing is the ingestion step that turns raw agent logs into staged `TraceRecord` JSONL files.

## What Runs Automatically

When `opentraces init` installs the agent capture hook, the hidden `_capture` command runs after each agent run ends. That capture path:

1. Finds new Claude Code session files under `~/.claude/projects/`
2. Parses the raw trace into a `TraceRecord`
...
```bash
opentraces discover          # list all Claude Code projects with trace files
opentraces parse             # parse all unparsed traces into staging
opentraces parse --auto      # parse and auto-approve (skip review)
opentraces parse --limit 10  # parse at most 10 traces
```
