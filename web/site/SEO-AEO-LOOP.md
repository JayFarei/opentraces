...
- **Deterministic SEO check, two modes** — `scripts/seo/seo-check.mjs`. BUILD mode (`--dist`) is the pre-deploy GATE wired into `.github/workflows/seo-checks.yml`; LIVE mode (`--url`) fetches the deployed site and is the scheduled MONITOR. Per priority page: title/description/canonical/indexable (meta-robots), JSON-LD validity + light name parity + **no-misframing on curated nodes** (rejects a "centralized SaaS" drift) + **SoftwareApplication fact allowlist** (MIT license, Python, free); robots real `Allow:/` + **no blanket `Disallow`** + **Content-Signal=yes** (values, not presence); sitemap presence + date differentiation. ~70 checks green in both modes. Hardened against the false-negatives an adversarial pass found (value-blind robots checks, one-word parity, whole-HTML noindex grep).
- **Scheduled monitor (the 2–3 day cron)** — `.github/workflows/seo-monitor.yml`. Every ~2 days it runs LIVE-mode `seo-check --url --write` against production, appends a `seo_check` standings record, commits it (`[skip ci]`), and goes **RED on a regression** (the alert). No build/credentials needed (Node `fetch` only). Credentialed signals (GSC, GEO panel, CrUX) and the crawler log feed slot in here as added steps in Phase 1/2.
- **§3 idempotency (dedupe/no-op)** — `recordExists()` guards `seo-check --write` and `crawler-report`: a second run for the same `(run_date_bucket, deploy_sha, surface)` is a no-op (`--force` overrides), so a duplicate/late scheduler fire never double-appends. (`pending`-resumption is still deferred.)
...
**Deferred (needs infra / credentials — NOT done despite §2/§3/§7 phrasing):**
- **Claude Code headless executor** for the LLM-driven propose/experiment step — the scheduled monitor (above) is deterministic node scripts; the §4 experiment loop's agent executor is Phase 1+.
- **§3 `pending`-resumption** — the dedupe/no-op is wired (above); resuming a crashed mid-run by re-deriving `pending` surfaces is still deferred.
- **Content-hash sitemap `lastmod` honesty** — the check currently asserts only "no `new Date()` in source + differentiated dates" (a weaker smoke test), NOT "`lastmod` advanced only when the content hash changed". The real content-hash check (§2/§3) is deferred.
