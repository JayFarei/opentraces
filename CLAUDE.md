...
- **Trace Trails**: VCS-anchored, append-only `TrailEvent` log under `refs/opentraces/local/events/v1` (snapshot refs are advisory projections); two-tier anchor identity (exact hash, then structural ≥ 0.85); survival states (`alive_on_path` … `repaired`); `ot://` resource scheme; anchor search recorded as ONE v2 summary event per commit/reconcile-run (plan 090), read via dual-shape `iter_search_records`. Visible surface: `trail blame commit <sha>`, `trail pr render|create|update`, `trail graph`, `trail track`.
