...
**Relationship:** opentraces is a superset of ATIF. We adopt step-based model, role conventions (`system | user | agent`), and field patterns. We add: attribution blocks, per-step token breakdowns, environment metadata, dependency tracking, security metadata. Export to ATIF via `opentraces export --format atif`.
