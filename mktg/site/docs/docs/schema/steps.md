...
| `subagent` | Sub-agent invocation (Explore, Plan) |
...
Sub-agent transcripts are linked by `session_id` reference to separate trajectory records (ATIF pattern), not embedded.
